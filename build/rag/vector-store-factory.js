...
import { VectorStoreError, VectorStoreLogger, validateVectorStoreConfig } from './vector-store-interface.js';
...
export class VectorStoreFactory {
    ...
    static create(config) {
        ...
        if (errors.length > 0) {
            throw new VectorStoreError(`Configuration invalide: ${errors.join(', ')}`, 'INVALID_CONFIG', { errors });
        }
        ...
        try {
            switch (config.type) {
                ...
                default:
                    throw new VectorStoreError(`Type de backend non supporté: ${config.type}`, 'UNSUPPORTED_BACKEND', { type: config.type });
            }
        }
        catch (error) {
            VectorStoreLogger.error('factory.create', `Échec de création du vector store`, error, {
                type: config.type
            });
     ...
     */
    static createSQLiteStore(config) {
     ...
     */
    static createPostgreSQLStore(config) {
        ...
        try {
            ...
            VectorStoreLogger.error('factory.postgresql', 'Échec de chargement du module PostgreSQL', error);
            throw new VectorStoreError('PostgreSQL non disponible. Assurez-vous que le module est installé et configuré.', 'POSTGRESQL_UNAVAILABLE', { error: error instanceof Error ? error.message : String(error) });
        }
    }
    /**
     ...
     */
    static createFallbackStore(originalConfig) {
     ...
     */
    static createFromProjectConfig(projectPath) {
        ...
        try {
            ...
            VectorStoreLogger.error('factory.project', `Échec de chargement de la configuration du projet`, error, {
                projectPath
            });
