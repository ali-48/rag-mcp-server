 ...
 */
export const pipelineValidatorTool = {
 ...
 */
export const pipelineValidatorHandler = async (args) => {
    ...
    try {
        ...
        const validationResults = {
            ...
            dependencies: { valid: false, errors: [] },
            io_compatibility: { valid: false, errors: [] },
            workflows: { valid: false, errors: [] },
            overall: { valid: false, warnings: [] }
        };
            ...
            for (const phase of phases) {
                ...
                for (const input of phase.inputs || []) {
                    ...
                    if (!found && phase.inputs.length > 0) {
                        validationResults.io_compatibility.errors.push(`Input '${input}' de la phase '${phase.id}' n'est produit par aucune phase dépendante`);
                    }
                }
                ...
                for (const output of phase.outputs || []) {
                    ...
                    if (conflictPhase) {
                        validationResults.io_compatibility.errors.push(`Output '${output}' de la phase '${phase.id}' en conflit avec la phase '${conflictPhase}'`);
                    }
                }
            }
        ...
        if (validateWorkflows && pipelineData.workflows && pipelineData.phases) {
            const workflows = pipelineData.workflows;
            const phaseIds = new Set(pipelineData.phases.map((p) => p.id));
            for (const workflow of workflows) {
                // Vérifier que toutes les phases du workflow existent
                for (const phaseId of workflow.phases) {
                    if (!phaseIds.has(phaseId)) {
                        validationResults.workflows.errors.push(`Phase '${phaseId}' dans le workflow '${workflow.id}' n'existe pas`);
                    }
                }
                ...
                for (let i = 0; i < phaseOrder.length; i++) {
                    ...
                    for (const depId of deps) {
                        ...
                        if (depIndex > i) {
                            validationResults.workflows.errors.push(`Workflow '${workflow.id}': Phase '${phaseId}' dépend de '${depId}' mais vient avant dans l'ordre`);
                        }
                    }
                }
            }
 ...
 */
function checkDependencyCycles(dependencies) {
