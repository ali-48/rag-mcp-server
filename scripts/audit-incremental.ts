...
import { AstCacheManager, createAstCacheManager } from "./ast-cache-manager.js";
import {
  FileChange,
  FileChangeType,
  FileQualityMetrics,
  IncrementalAuditConfig,
 ...
 */
const DEFAULT_CONFIG: IncrementalAuditConfig = {
  rootDir: process.cwd(),
  useAstCache: true,
...
export class IncrementalAuditor {
  private config: IncrementalAuditConfig;
  private cacheManager: AstCacheManager;
  private auditId: string;

  constructor(config?: Partial<IncrementalAuditConfig>) {
   ...
   */
  async auditFilesIncremental(
    files: string[],
  ): Promise<IncrementalAuditResult> {
    ...
    console.log(
      `📊 Changements détectés: ${fileChanges.filter((c) => c.changeType !== "unchanged").length}`,
    );
    ...
    const result: IncrementalAuditResult = {
    ...
    console.log(
      `📈 Score de qualité: ${statistics.currentQualityScore?.toFixed(2) || "N/A"}`,
    );
   ...
   */
  private async analyzeFileChanges(files: string[]): Promise<FileChange[]> {
   ...
   */
  private async analyzeFileChange(filePath: string): Promise<FileChange> {
    try {
      ...
      let oldModified: Date | undefined;
      let oldQuality: FileQualityMetrics | undefined;
      ...
      console.error(
        `[IncrementalAuditor] Erreur lors de l'analyse du changement pour ${filePath}:`,
        error,
      );
   ...
   */
  private async analyzeFilesInDetail(
    files: string[],
    ...
    const promises = files.map(async (filePath) => {
      try {
        ...
        if (fileChange && analysisResult) {
          ...
          fileChange.symbolDiffs = await this.compareSymbols(
            filePath,
        ...
        console.error(
          `[IncrementalAuditor] Erreur lors de l'analyse détaillée de ${filePath}:`,
          error,
        );
      }
    });
   ...
   */
  private async analyzeFileWithCodeMapper(filePath: string): Promise<any> {
    try {
      ...
      console.error(
        `[IncrementalAuditor] Erreur avec code-mapper pour ${filePath}:`,
        error,
      );
      return null;
    }
  ...
  private async compareSymbols(
    ...
    try {
      ...
      const currentSymbolMap = new Map<string, SymbolInfo>();
      const cachedSymbolMap = new Map<string, SymbolInfo>();

      currentSymbols.forEach((s) =>
        currentSymbolMap.set(this.getSymbolKey(s), s),
      );
      cachedSymbols.forEach((s) =>
        cachedSymbolMap.set(this.getSymbolKey(s), s),
      );
      ...
      for (const [key, currentSymbol] of currentSymbolMap) {
        ...
        if (
          ...
          diffs.push({
            ...
            severity: this.getSymbolChangeSeverity(
              "modified",
              currentSymbol,
              cachedSymbol,
            ),
      ...
      console.error(
        `[IncrementalAuditor] Erreur lors de la comparaison des symboles pour ${filePath}:`,
        error,
      );
      return [];
    }
   ...
   */
  private getSymbolKey(symbol: SymbolInfo): string {
   ...
   */
  private getSymbolChangeSeverity(
  ...
  private generateStatistics(
    fileChanges: FileChange[],
  ): IncrementalAuditResult["statistics"] {
    const addedFiles = fileChanges.filter(
      (c) => c.changeType === "added",
    ).length;
    const modifiedFiles = fileChanges.filter(
      (c) => c.changeType === "modified",
    ).length;
    const deletedFiles = fileChanges.filter(
      (c) => c.changeType === "deleted",
    ).length;
    const unchangedFiles = fileChanges.filter(
    ...
    fileChanges.forEach((change) => {
      change.symbolDiffs.forEach((diff) => {
        if (diff.changeType === "added") addedSymbols++;
        if (diff.changeType === "modified") modifiedSymbols++;
        if (diff.changeType === "deleted") deletedSymbols++;
    ...
    const averageQualityImpact =
      qualityImpactCount > 0 ? totalQualityImpact / qualityImpactCount : 0;
    ...
    const previousAvg =
      qualityScoreCount > 0 ? previousQualityScore / qualityScoreCount : 0;
    const currentAvg =
      qualityScoreCount > 0 ? currentQualityScore / qualityScoreCount : 0;
    ...
    return {
      ...
      qualityScoreDelta:
        qualityScoreCount > 0 ? currentAvg - previousAvg : undefined,
    };
  }
  ...
  private generateRecommendations(
    fileChanges: FileChange[],
    statistics: IncrementalAuditResult["statistics"],
  ): IncrementalAuditResult["recommendations"] {
    const recommendations: IncrementalAuditResult["recommendations"] = [];
    ...
    if (
      statistics.qualityScoreDelta !== undefined &&
   ...
   */
  private async exportResults(result: IncrementalAuditResult): Promise<void> {
    try {
      ...
      console.error(
        `[IncrementalAuditor] Erreur lors de l'export des résultats:`,
        error,
      );
    }
  }
 ...
 */
export async function auditFilesIncremental(
  files: string[],
  config?: Partial<IncrementalAuditConfig>,
): Promise<IncrementalAuditResult> {
 ...
 */
export async function auditFileIncremental(
  filePath: string,
  config?: Partial<IncrementalAuditConfig>,
): Promise<IncrementalAuditResult> {
 ...
 */
export async function auditDirectoryIncremental(
  directoryPath: string,
  config?: Partial<IncrementalAuditConfig>,
): Promise<IncrementalAuditResult> {
  // Cette fonctionnalité serait à implémenter avec glob
  console.log(
    `[auditDirectoryIncremental] Non implémenté pour ${directoryPath}`,
  );
