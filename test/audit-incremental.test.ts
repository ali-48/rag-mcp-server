...
import {
  auditFileIncremental,
  auditFilesIncremental,
} from "../scripts/audit-incremental.js";
...
describe("Audit Incrémental", () => {
  ...
  describe("Gestionnaire de cache AST", () => {
    ...
    it("devrait sauvegarder et récupérer des entrées du cache", async () => {
      ...
      const saveResult = await cacheManager.set(
        testFilePath,
        '{"type": "Program"}',
        [{ name: "test", type: "variable", startLine: 1, endLine: 1 }],
        { qualityScore: 0.8 },
      );
  ...
  describe("Configuration", () => {
    it("devrait accepter une configuration personnalisée", async () => {
      ...
      const customConfig = {
        ...
        trackedSymbolTypes: ["function" as const, "class" as const],
