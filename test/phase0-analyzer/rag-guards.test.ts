...
import {
  ...
  requireQueryReady,
  requireScan
} from '../../src/rag/guards/rag-guards-refactored.js';
...
describe('rag-guards (refactored)', () => {
  ...
  describe('checkRagPhase', () => {
    it('should pass when project is initialized', async () => {
      ...
      (getRagState as any).mockResolvedValue(mockState);

      const requirements: RagPhaseRequirements = { initialized: true };
  ...
  describe('isPhaseReady', () => {
    ...
    it('should delegate to checkJobRequirements', async () => {
      ...
      const mockResult: GuardResult = { passed: true };
