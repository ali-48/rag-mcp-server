...
import { RagJob, RagJobStatus, RagJobType } from '../../src/rag/queue/job-types.js';
import { RagQueue, RagQueueConfig, getRagQueue, testRagQueue } from '../../src/rag/queue/rag-queue.js';
...
describe("Tests unitaires pour la file d'attente RAG", () => {
    let queue: RagQueue;
    const defaultConfig: Partial<RagQueueConfig> = {
        maxQueueSize: 5,
    ...
    describe('Initialisation', () => {
        ...
        it('devrait accepter une configuration personnalisée', () => {
            const customConfig: Partial<RagQueueConfig> = {
                maxQueueSize: 10,
    ...
    describe('Exclusivité des mutateurs', () => {
        it('devrait exécuter un seul job mutateur à la fois', async () => {
            const mutatorJobs: RagJob[] = [
                {
                    id: 'mutator-1',
                    type: 'scan' as RagJobType, // scan est un mutateur
                    status: 'pending' as RagJobStatus,
                    projectPath: '/test/project',
                    createdAt: new Date(),
                    priority: 1
                ...
                {
                    id: 'mutator-2',
                    type: 'prepare' as RagJobType, // prepare est un mutateur
                    status: 'pending' as RagJobStatus,
                    projectPath: '/test/project',
                    createdAt: new Date(),
                    priority: 1
        ...
        it('devrait permettre plusieurs jobs en lecture seule simultanément', async () => {
            const readOnlyJobs: RagJob[] = [
                {
                    id: 'query-1',
                    type: 'query' as RagJobType, // query est en lecture seule
                    status: 'pending' as RagJobStatus,
                    projectPath: '/test/project',
                    createdAt: new Date(),
                    priority: 1
        ...
        it('devrait bloquer les mutateurs quand un mutateur est déjà en cours', async () => {
            ...
            const secondMutator: RagJob = {
                id: 'mutator-second',
                type: 'prepare' as RagJobType,
                status: 'pending' as RagJobStatus,
                projectPath: '/test/project',
                createdAt: new Date(),
                priority: 1
        ...
        it('devrait lister les jobs par type et statut', async () => {
            const jobs: RagJob[] = [
                ...
                },
                {
                    id: 'scan-running',
                    type: 'scan' as RagJobType,
                    status: 'running' as RagJobStatus,
                    projectPath: '/test/project',
                    createdAt: new Date(),
                    startedAt: new Date(),
                    priority: 1
    ...
    describe('Annulation de jobs', () => {
        ...
        it('devrait échouer à annuler un job en cours d\'exécution', async () => {
            const job: RagJob = {
                id: 'running-job',
                type: 'scan' as RagJobType,
                status: 'running' as RagJobStatus, // Statut running
                projectPath: '/test/project',
                createdAt: new Date(),
                startedAt: new Date(),
                priority: 1
        ...
        it('devrait échouer à annuler un job déjà terminé', async () => {
            const job: RagJob = {
                id: 'completed-job',
                type: 'scan' as RagJobType,
                status: 'done' as RagJobStatus, // Statut terminé
                projectPath: '/test/project',
                createdAt: new Date(),
