...
import { ToolDefinition, ToolHandler, toolRegistry } from "./tool-registry.js";

/**
 * Interface pour un module d'outil
 */
...
/**
 * Configuration du registre v2.0
 */
export interface RegistryConfigV2 {
    toolDirectories: string[];
    toolFilePattern: RegExp;
    toolExportPattern: RegExp;
    verbose?: boolean;
    legacyMode?: boolean;
    exposedTools?: string[];
    hiddenTools?: string[];
...
export class AutoRegistryV2 {
    private config: RegistryConfigV2;
    private registeredTools: Set<string> = new Set();

    constructor(config: Partial<RegistryConfigV2> = {}) {
     ...
     */
    async discoverToolModules(): Promise<Array<{ path: string; module: ToolModule }>> {
        const modules: Array<{ path: string; module: ToolModule }> = [];
     ...
     */
    private applyVisibility(tool: ToolDefinition): ToolDefinition {
     ...
     */
    listRegisteredTools(): Array<{ name: string; hidden: boolean }> {
        const allTools = toolRegistry.getTools();
     ...
     */
    configureFromConfig(configData: any): void {
     ...
     */
    private updateConfig(newConfig: RegistryConfigV2): void {
 ...
 */
export async function initializeAutoRegistryV2(config?: Partial<RegistryConfigV2>): Promise<number> {
