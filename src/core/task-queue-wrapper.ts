...
import { createIndexTask, QueuedTask, TaskQueue } from './task-queue.js';
...
export class TaskQueueWrapper {
  ...
  private eventEmitter: ReturnType<typeof getTaskEventEmitter>;
   ...
   */
  private async runNextWithEvents(projectPath: string): Promise<void> {
    const startTime = Date.now();
  ...
  getRunningCount(projectPath: string) {
  ...
  waitForCompletion(taskId: string, timeoutMs?: number, pollIntervalMs?: number) {
   ...
   */
  configureEventEmitter(options: any): void {
 ...
 */
let taskQueueWrapper: TaskQueueWrapper | null = null;

/**
 * Obtient l'instance singleton du TaskQueueWrapper
 */
export function getTaskQueueWrapper(): TaskQueueWrapper {
  if (!taskQueueWrapper) {
 ...
 */
export async function testTaskQueueWrapper(): Promise<boolean> {
  try {
