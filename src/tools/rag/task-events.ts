...
import { getTaskEventEmitter, TaskEvent, TaskEventType } from "../../core/task-event-emitter.js";
import { ToolDefinition, ToolHandler } from "../../core/tool-registry.js";

/**
 * Interface pour un abonnement aux événements
 */
...
class EventSubscriptionManager {
  ...
  private eventHistory = new Map<string, TaskEvent[]>();
  private readonly MAX_HISTORY_PER_SUBSCRIPTION = 100;

  /**
   * Crée un nouvel abonnement
   */
  createSubscription(eventTypes: TaskEventType[], projectPath?: string): EventSubscription {
   ...
   */
  addEventToSubscription(subscriptionId: string, event: TaskEvent): void {
   ...
   */
  getSubscriptionEvents(subscriptionId: string, limit: number = 50): TaskEvent[] {
    const history = this.eventHistory.get(subscriptionId) || [];
   ...
   */
  cleanupInactiveSubscriptions(maxAgeHours: number = 24): number {
    const now = Date.now();
  ...
  getStats(): {
    ...
    total_events: number;
    by_event_type: Record<TaskEventType, number>;
    by_project: Record<string, number>;
  } {
    const stats = {
      total_subscriptions: this.subscriptions.size,
      total_events: 0,
      by_event_type: {} as Record<TaskEventType, number>,
 ...
 */
export const subscribeTaskEventsTool: ToolDefinition = {
 ...
 */
export const subscribeTaskEventsHandler: ToolHandler = async (args) => {
  ...
  try {
    ...
    // Créer l'abonnement
    const subscription = subscriptionManager.createSubscription(eventTypes as TaskEventType[], projectPath);
    ...
    eventTypes.forEach((eventType: TaskEventType) => {
      eventEmitter.on(eventType, (event: TaskEvent) => {
 ...
 */
export const getTaskEventsTool: ToolDefinition = {
 ...
 */
export const getTaskEventsHandler: ToolHandler = async (args) => {
  const startTime = Date.now();
 ...
 */
export const unsubscribeTaskEventsTool: ToolDefinition = {
 ...
 */
export const unsubscribeTaskEventsHandler: ToolHandler = async (args) => {
  const startTime = Date.now();
 ...
 */
export async function testTaskEvents(): Promise<boolean> {
  try {
    const eventEmitter = getTaskEventEmitter();
