    ...
    try {
        ...
        if (isCommandExecuted && !input.force) {
            return formatErrorResponse(
                ...
                'init_rag a déjà été exécuté. Utilisez force=true pour forcer une ré-exécution.',
                {
        ...
        if (alreadyInitialized && !input.force) {
            return formatErrorResponse(
                'ALREADY_INITIALIZED',
                'Le projet est déjà initialisé pour RAG. Utilisez force=true pour réinitialiser.',
                {
                    requiredAction: 'use_force_true',
                    notesForAI: [
 ...
 */
export const initRagTool: ToolDefinition = {
    name: "init_rag",
