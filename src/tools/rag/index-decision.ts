...
export const indexDecisionTool: ToolDefinition = {
         ...
         */
        export function parseTaskDecision(rawData: any): {
          task_id: string;
      metadata: Record<string, any>;
      decision: {
        type: string;
     ...
     */
    export function validateParsedDecision(parsedData: any): {
      valid: boolean;
      errors: string[];
      warnings: string[];
    } {
     ...
     */
    export function convertToTaskDecision(parsedData: any): TaskDecision {
      ...
      const taskDecision: TaskDecision = {
        ...
        decision_type: parsedData.decision.type as any,
      ...
      Object.keys(taskDecision.decision_data).forEach(key => {
        if (taskDecision.decision_data[key as keyof typeof taskDecision.decision_data] === undefined) {
          delete taskDecision.decision_data[key as keyof typeof taskDecision.decision_data];
        }
      });

      if (taskDecision.context) {
        Object.keys(taskDecision.context).forEach(key => {
          if (taskDecision.context![key as keyof typeof taskDecision.context] === undefined) {
            delete taskDecision.context![key as keyof typeof taskDecision.context];
          }
        });
