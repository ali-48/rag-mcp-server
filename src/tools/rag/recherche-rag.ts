...
class RechercheRagLogger {
    private static instance: RechercheRagLogger;
    private logLevel: LogLevel = LogLevel.ERROR;

    private constructor() { }

    static getInstance(): RechercheRagLogger {
 ...
 */
export const rechercheRagTool: ToolDefinition = {
    name: "recherche_rag",
 ...
 */
export const rechercheRagHandler: ToolHandler = async (args) => {
 ...
 */
function prepareLegacyResponse(
    searchResults: any[],
    metadata: any,
    logger: RechercheRagLogger
): { content: Array<{ type: string; text: string }> } {
...
function formatHumanReadable(
    ...
    results.forEach((result, index) => {
        ...
        if (result.metadata) {
            ...
            if (result.metadata.language) output += ` (${result.metadata.language})`;
            output += `\n`;
