 ...
 */
export const prepareRagTool: ToolDefinition = {
  name: "prepare_rag",
  description:
    "Phase 1: Préparation des fichiers pour l'embedding (chunking, nettoyage, normalisation)",
  inputSchema: {
    type: "object",
    properties: {
      project_path: {
 ...
 */
export const embedRagTool: ToolDefinition = {
  name: "embed_rag",
...
export const indexRagTool: ToolDefinition = {
  name: "index_rag",
  description:
    "Phase 2-4: Indexation asynchrone avec chunking intelligent, embeddings et suivi de progression",
  inputSchema: {
    type: "object",
    properties: {
      project_path: {
      ...
      wait_for_completion: {
        type: "boolean",
        description:
          "Attendre la complétion de la tâche (false = retourne task_id immédiatement)",
        default: false,
      },
      timeout_seconds: {
        type: "number",
...
async function indexTask(
  ...
  try {
    ...
    const provider = defaults.embedding_provider as
      | "ollama"
      | "fallback"
    ...
    progressTracker.updateEmbeddingCost(
      taskId,
      estimatedTokens,
      embeddingModel,
      Math.ceil(estimatedTokens / 1000), // Estimation: 1000 tokens/seconde
    );

    logger.info(
      "rag.index.task.start",
      `Début de la tâche d'indexation: ${taskId}`,
      {
        taskId,
        projectPath,
        mode,
    ...
    const indexOptions = {
      ...
      onProgress: (
        ...
        if (filesProcessed % 10 === 0) {
          logger.info(
            "rag.index.task.progress",
            `Progression indexation: ${filesProcessed}/${filesTotal}`,
            {
              taskId,
              filesProcessed,
              filesTotal,
    ...
    progressTracker.update(taskId, {
      step: "execution",
      currentOperation:
        mode === "incremental" ? "incremental_indexing" : "full_indexing",
    });

    let indexResult;
    if (mode === "incremental") {
      logger.info(
        "rag.index.task.mode.incremental",
        `Mode incrémental: ${taskId}`,
      );
    ...
    logger.info(
      "rag.index.task.complete",
      `Tâche d'indexation terminée: ${taskId}`,
      {
        taskId,
    ...
    logger.error(
      "rag.index.task.error",
      `Erreur dans la tâche d'indexation: ${taskId}`,
      {
        taskId,
 ...
 */
async function estimateEmbeddingTokens(
  projectPath: string,
  options: any,
): Promise<number> {
  try {
    ...
    const filePatterns = options.file_patterns || [
      "**/*.{js,ts,py,md,txt,json,yaml,yml,html,css,scss}",
    ];
    const recursive =
    ...
    logger.warn(
      "rag.index.estimation.error",
      "Erreur lors de l'estimation des tokens",
      {
        error: error instanceof Error ? error.message : String(error),
      },
    );
...
async function detectProjectPath(): Promise<string> {
  ...
  throw new Error(
    "Impossible de détecter automatiquement le projet. Spécifiez 'project_path'.",
  );
}
 ...
 */
async function estimateFileCount(
  projectPath: string,
  options: any,
): Promise<number> {
  try {
    ...
    logger.warn(
      "rag.index.estimation.files.error",
      "Erreur lors de l'estimation du nombre de fichiers",
      {
        error: error instanceof Error ? error.message : String(error),
      },
    );
  ...
  try {
    ...
    if (!projectPath) {
      try {
        ...
        logger.info(
          "rag.index.project.auto_detected",
          `Projet auto-détecté: ${projectPath}`,
          { path: projectPath },
        );
      } catch (error) {
        const errorMessage =
        ...
        logger.error(
          "rag.index.project.detection_error",
    ...
    if (!isInitialized) {
      ...
      logger.error("rag.index.not_initialized", errorMessage, {
    ...
    if (!enqueueResult.queued) {
      throw new Error(
        `Impossible d'ajouter le job à la file d'attente: ${enqueueResult.message}`,
      );
    }
    ...
    const asyncResponse: AsyncRagResponse = {
      status: "accepted",
      action: "index_rag",
      task_id: job.id,
      execution: "background",
      message:
        "Indexation démarrée en arrière-plan. Utilisez get_status pour suivre la progression.",
 ...
 */
export const embedRagHandler: ToolHandler = async (args) => {
  const startTime = Date.now();
  ...
  try {
    ...
    if (!projectPath) {
      try {
        ...
        if (hasProjectFile) {
          projectPath = cwd;
          logger.info(
            "rag.embed.project.auto_detected",
            `Projet auto-détecté: ${projectPath}`,
            { path: projectPath },
          );
        } else {
        ...
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        logger.error(
          "rag.embed.project.detection_error",
          "Erreur de détection automatique",
          { error: errorMessage },
        );
        throw error;
    ...
    if (!isInitialized) {
      ...
      logger.error("rag.embed.not_initialized", errorMessage, {
        project_path: projectPath,
      });
    ...
    const asyncResponse: AsyncRagResponse = {
      ...
      execution: "background",
      message:
        "Génération d'embeddings démarrée en arrière-plan. Utilisez get_status pour suivre la progression.",
      next_action: "get_status",
      notes_for_ai: [
    ...
    logger.error(
      "rag.embed.error",
      "Erreur lors de la création du job d'embedding",
      {
        error: error.message,
        stack: error.stack,
        duration: `${duration}s`,
 ...
 * Retourne immédiatement un task_id et crée un job dans la file d'attente RAG
 */
export const prepareRagHandler: ToolHandler = async (args) => {
  const startTime = Date.now();
  ...
  try {
    ...
    if (!projectPath) {
      try {
        ...
        if (hasProjectFile) {
          projectPath = cwd;
          logger.info(
            "rag.prepare.project.auto_detected",
            `Projet auto-détecté: ${projectPath}`,
            { path: projectPath },
          );
        } else {
        ...
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        logger.error(
          "rag.prepare.project.detection_error",
          "Erreur de détection automatique",
          { error: errorMessage },
        );
        throw error;
    ...
    if (!isInitialized) {
      ...
      logger.error("rag.prepare.not_initialized", errorMessage, {
        project_path: projectPath,
      });
    ...
    const asyncResponse: AsyncRagResponse = {
      ...
      execution: "background",
      message:
        "Préparation démarrée en arrière-plan. Utilisez get_status pour suivre la progression.",
      next_action: "get_status",
      notes_for_ai: [
    ...
    logger.error(
      "rag.prepare.error",
      "Erreur lors de la création du job de préparation",
      {
        error: error.message,
        stack: error.stack,
        duration: `${duration}s`,
