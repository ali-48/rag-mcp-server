...
async function indexTask(
  ...
  try {
    ...
    const provider = defaults.embedding_provider as "ollama" | "fake" | "sentence-transformers";
    ...
    const indexOptions = {
      ...
      onProgress: (filesProcessed: number, filesTotal: number, currentFile?: string) => {
        progressTracker.update(taskId, {
          step: 'indexing',
          filesProcessed,
          filesTotal,
 ...
 */
async function estimateEmbeddingTokens(projectPath: string, options: any): Promise<number> {
  try {
 ...
 */
async function estimateFileCount(projectPath: string, options: any): Promise<number> {
  try {
    const fg = await import('fast-glob');
