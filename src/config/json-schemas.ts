...
import { JSONSchema } from '../core/json-schema-validator.js';

/**
 * Schéma JSON pour rag-config.json
 */
export const RAG_CONFIG_SCHEMA: JSONSchema = {
 ...
 */
export const DB_CONFIG_SCHEMA: JSONSchema = {
    $schema: 'http://json-schema.org/draft-07/schema#',
    type: 'object',
 ...
 */
export const STATE_SCHEMA: JSONSchema = {
    $schema: 'http://json-schema.org/draft-07/schema#',
    type: 'object',
    title: 'State Schema',
 ...
 */
export const PIPELINE_SCHEMA: JSONSchema = {
    $schema: 'http://json-schema.org/draft-07/schema#',
    type: 'object',
...
export class ConfigValidator {
     ...
     */
    async validateRagConfig(config: any): Promise<{ valid: boolean; errors?: string[] }> {
        const validator = await this.getValidator();
        const result = validator.validate(config, RAG_CONFIG_SCHEMA);
        return {
            valid: result.valid,
            errors: result.errors.map((e: any) => `${e.path}: ${e.message}`)
        };
    }
     ...
     */
    async validateDbConfig(config: any): Promise<{ valid: boolean; errors?: string[] }> {
        const validator = await this.getValidator();
     ...
     * Valide un pipeline
     */
    async validatePipeline(pipeline: any): Promise<{ valid: boolean; errors?: string[] }> {
        const validator = await this.getValidator();
    ...
    async validateAllConfigs(): Promise<{
        rag_config: { valid: boolean; errors: string[] };
        db_config: { valid: boolean; errors: string[] };
        state: { valid: boolean; errors: string[] };
        pipeline: { valid: boolean; errors: string[] };
    }> {
