...
export interface ErrorHandlerConfig {
  ...
  recoveryStrategies: Array<
    "retry" | "fallback" | "circuit-breaker" | "degraded"
  >;
...
export class ErrorHandler {
  ...
  private recoveryStrategies = new Map<
    string,
    (error: Error, context?: ErrorContext) => Promise<any>
  >();
  ...
  private initRecoveryStrategies(): void {
    ...
    this.recoveryStrategies.set("fallback", async (error, context) => {
      logger.info(
        "error.handler.recovery.fallback",
    ...
    this.recoveryStrategies.set("circuit-breaker", async (error, context) => {
      logger.warn(
        "error.handler.recovery.circuit_breaker",
  ...
  formatForHuman(
    ...
    // Temps estimé de résolution
    humanError.estimatedResolutionTime = this.estimateResolutionTime(
      severity,
      category,
    );
  ...
  handleError(
    ...
    if (
      ...
      this.shouldAttemptRecovery(error, severity)
    ) {
  ...
  private async attemptRecovery(
    ...
    for (const strategyName of this.config.recoveryStrategies) {
      ...
      try {
        const result = await Promise.race([
          ...
          new Promise((_, reject) =>
            setTimeout(
              () =>
                reject(
                  new Error(`Timeout récupération stratégie ${strategyName}`),
                ),
        ...
        logger.warn(
          "error.handler.recovery.strategy_failed",
          "Stratégie de récupération échouée",
          {
            strategy: strategyName,
            error:
    ...
    logger.error(
      "error.handler.recovery.all_failed",
      "Toutes les stratégies de récupération ont échoué",
      {
        error: error.message,
  ...
  private generateErrorCode(error: Error): string {
    ...
    const severity = this.determineSeverity(error)
      .substring(0, 1)
      .toUpperCase();
    const hash = this.hashString(error.message + (error.stack || "")).substring(
      0,
      6,
    );
  ...
  private generateSuggestions(error: Error, category: ErrorCategory): string[] {
    ...
    switch (category) {
      ...
      case "network":
        ...
        suggestions.push(
          "Assurez-vous que les services distants sont accessibles",
        );
        break;
      case "database":
        suggestions.push("Vérifiez la connexion à la base de données");
        suggestions.push(
          "Assurez-vous que les tables existent et sont accessibles",
        );
        break;
      case "memory":
   ...
   */
  private limitErrorSize<T extends MCPErrorFormat | HumanErrorFormat>(
    error: T,
  ): T {
    ...
    if ("stackTrace" in error && error.stackTrace) {
      error.stackTrace =
        error.stackTrace.substring(0, 1000) + "... [TRUNCATED]";
    }
  ...
  private generatePossibleCauses(
    ...
    if (context?.executionPhase) {
      causes.push(
        `Erreur survenue pendant la phase: ${context.executionPhase}`,
      );
    }
  ...
  private determineUserImpact(
    ...
    const impacts: Record<string, string> = {
      ...
      warning:
        "Impact modéré - certaines fonctionnalités peuvent être limitées",
  ...
  private generateCodeExamples(
    error: Error,
    category: ErrorCategory,
  ): Array<{ language: string; code: string; description: string }> {
...
const validData = {
  ...
  private updateStats(
    ...
    const errorCode = this.generateErrorCode(error);
    const existingError = this.stats.recurringErrors.find(
      (e) => e.code === errorCode,
    );
  ...
  private checkAlertThresholds(): void {
    ...
    if (this.errorHistory.length >= alertThresholds.consecutiveErrors) {
      const recentErrors = this.errorHistory.slice(
        -alertThresholds.consecutiveErrors,
      );
      const allRecentAreErrors = recentErrors.every(
        (entry) =>
      ...
      if (allRecentAreErrors) {
        logger.warn(
          "error.handler.alert.consecutive_errors",
          "Erreurs consécutives détectées",
          {
    ...
    if (typeof process !== "undefined" && process.memoryUsage) {
      ...
      if (usagePercentage > alertThresholds.memoryUsage) {
        logger.warn(
          "error.handler.alert.memory_usage",
  ...
  clearErrorHistory(): void {
    this.errorHistory = [];
    logger.info(
      "error.handler.history.cleared",
      "Historique des erreurs effacé",
    );
  }
 ...
 */
export function formatErrorForHuman(
  error: Error,
  context?: ErrorContext,
): HumanErrorFormat {
  const handler = getErrorHandler();
