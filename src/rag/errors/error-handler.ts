...
/**
 * Catégorie d'erreur pour le regroupement
 */
...
/**
 * Format d'erreur pour MCP (JSON strict)
 */
export interface MCPErrorFormat {
    ...
    /** Message utilisateur (localisé si besoin) */
    userMessage?: string;

    /** Action requise pour résoudre */
...
/**
 * Format d'erreur pour humains (lisible)
 */
export interface HumanErrorFormat {
    /** Titre de l'erreur */
    title: string;
...
/**
 * Contexte d'erreur pour enrichissement
 */
...
export interface ErrorFormattingOptions {
    ...
    /** Cible (MCP, humain, log) */
    target?: 'mcp' | 'human' | 'log' | 'all';
...
export interface ErrorStats {
    ...
    /** Taux d'erreur (erreurs/requêtes) */
    errorRate: number;
...
export interface ErrorHandlerConfig {
    /** Activer le formatage MCP */
    enableMCPFormatting: boolean;

    /** Activer le formatage humain */
    enableHumanFormatting: boolean;

    /** Activer les logs structurés */
    enableStructuredLogging: boolean;

    /** Activer la récupération d'erreurs */
    ...
    defaultLogLevel: ErrorSeverity;

    /** Formats de sortie par défaut */
    defaultOutputFormats: Array<'json' | 'text' | 'html' | 'markdown'>;
    ...
    /** Limite de taille des erreurs */
    maxErrorSize: number;

    /** Timeout de récupération (ms) */
    recoveryTimeout: number;

    /** Stratégies de récupération */
    recoveryStrategies: Array<'retry' | 'fallback' | 'circuit-breaker' | 'degraded'>;
...
export class ErrorHandler {
    ...
    private errorHistory: Array<{
        ...
        formatted?: MCPErrorFormat | HumanErrorFormat;
    }> = [];
    private recoveryStrategies = new Map<string, (error: Error, context?: ErrorContext) => Promise<any>>();
     ...
     */
    formatForMCP(error: Error, context?: ErrorContext, options?: ErrorFormattingOptions): MCPErrorFormat {
     ...
     */
    formatForHuman(error: Error, context?: ErrorContext, options?: ErrorFormattingOptions): HumanErrorFormat {
        const severity = this.determineSeverity(error);
        const category = this.determineCategory(error);
        ...
        let humanSeverity: 'low' | 'medium' | 'high' | 'critical';
        switch (severity) {
            case 'debug':
            case 'info':
                humanSeverity = 'low';
        ...
        const humanError: HumanErrorFormat = {
     ...
     */
    handleError(error: Error, context?: ErrorContext, options?: ErrorFormattingOptions): void {
        const severity = this.determineSeverity(error);
        const category = this.determineCategory(error);
        ...
        let mcpFormatted: MCPErrorFormat | undefined;
        let humanFormatted: HumanErrorFormat | undefined;
            ...
            try {
                const result = await Promise.race([
                    ...
                    new Promise((_, reject) =>
                        setTimeout(() => reject(new Error(`Timeout récupération stratégie ${strategyName}`)),
     ...
     */
    private generateUserMessage(error: Error, locale?: string): string {
     ...
     */
    private generateHelpLinks(error: Error, category: ErrorCategory): Array<{ title: string; url: string; description?: string }> {
     ...
     */
    private limitErrorSize<T extends MCPErrorFormat | HumanErrorFormat>(error: T): T {
     ...
     */
    private generateErrorTitle(error: Error, category: ErrorCategory): string {
     ...
     */
    private generatePossibleCauses(error: Error, category: ErrorCategory, context?: ErrorContext): string[] {
        const causes: string[] = [];
     ...
     */
    private generateResolutionSteps(error: Error, category: ErrorCategory, context?: ErrorContext): string[] {
        const steps: string[] = [];
     ...
     */
    private determineUserImpact(severity: ErrorSeverity, category: ErrorCategory): string {
     ...
     */
    private generateCodeExamples(error: Error, category: ErrorCategory): Array<{ language: string; code: string; description: string }> {
...
const validData = {
  ...
  mode: "full"
     ...
     */
    private estimateResolutionTime(severity: ErrorSeverity, category: ErrorCategory): string {
     ...
     */
    private updateStats(error: Error, severity: ErrorSeverity, category: ErrorCategory, context?: ErrorContext): void {
    ...
    private logStructuredError(
        ...
        context?: ErrorContext,
        mcpFormatted?: MCPErrorFormat,
        humanFormatted?: HumanErrorFormat
    ): void {
     ...
     */
    private shouldAttemptRecovery(error: Error, severity: ErrorSeverity): boolean {
    ...
    private checkAlertThresholds(): void {
        ...
        if (this.errorHistory.length >= alertThresholds.consecutiveErrors) {
            ...
            const allRecentAreErrors = recentErrors.every(entry =>
                this.determineSeverity(entry.error) === 'error' ||
     ...
     */
    getErrorHistory(): Array<{ timestamp: Date; error: Error; context?: ErrorContext }> {
 ...
 */
export function formatErrorForMCP(error: Error, context?: ErrorContext): MCPErrorFormat {
 ...
 */
export function formatErrorForHuman(error: Error, context?: ErrorContext): HumanErrorFormat {
    const handler = getErrorHandler();
 ...
 */
export async function testErrorHandlerModule(): Promise<boolean> {
