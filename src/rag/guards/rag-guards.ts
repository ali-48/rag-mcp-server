 ...
 */
function createMCPFormatFromError(
    error: RagUsageError,
    recommendations: string[] = []
): GuardResult['mcpFormat'] {
    const notesForAI = [
...
/**
 * Résultat de l'analyse des phases
 */
 ...
 */
export async function getPhaseAnalysis(projectPath: string): Promise<PhaseAnalysis> {
    try {
        const state = await getRagState(projectPath);
        const phases = [
        ...
        for (const phase of phases) {
            let status: 'done' | 'running' | 'pending' | 'error' = 'pending';
            if (phase.name === 'init') {
