 ...
 * Type de fichier
 */
export type FileType = 'code' | 'doc' | 'config' | 'other';

/**
 * Langage de programmation
 */
export type Language = 'ts' | 'js' | 'json' | 'md' | 'yml' | 'yaml' | 'toml' | 'sql' | 'sh' | 'txt' | 'html' | 'css' | 'unknown';
 ...
 * Type d'import
 */
export type ImportType = 'import' | 'require' | 'dynamic';
...
export class CodeAnalyzer {
   ...
   */
  async analyzeProject(rootDir: string): Promise<CodeMap> {
   ...
   */
  private async scanFiles(rootDir: string): Promise<FileInfo[]> {
   ...
   */
  private shouldExclude(filePath: string, isDirectory: boolean): boolean {
   ...
   */
  private getLanguage(filePath: string): Language | undefined {
    const ext = path.extname(filePath).toLowerCase();
   ...
   */
  private generateFileId(index: number): string {
   ...
   */
  private generateFunctionId(index: number): string {
   ...
   */
  private generateClassId(index: number): string {
   ...
   */
  private async analyzeTypeScriptFiles(files: FileInfo[]): Promise<void> {
    if (this.config.verbose) {
   ...
   */
  private extractExports(sourceFile: any): ExportInfo[] {
    const exports: ExportInfo[] = [];

    // Fonction pour déterminer le type d'un nœud
    const getNodeType = (node: any): SymbolKind => {
   ...
   */
  private extractFunctions(sourceFile: any, startIndex: number): FunctionInfo[] {
   ...
   */
  private extractClasses(sourceFile: any, startIndex: number): ClassInfo[] {
   ...
   */
  private async buildRelations(files: FileInfo[]): Promise<{
    imports: ImportRelation[];
    calls: CallRelation[];
    inheritance: InheritanceRelation[];
  }> {
   ...
   */
  private async calculateMetrics(files: FileInfo[]): Promise<void> {
    if (this.config.verbose) {
