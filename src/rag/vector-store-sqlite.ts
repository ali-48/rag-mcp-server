...
export class VectorStoreSQLite implements IVectorStore {
  ...
  async deleteDocumentsByPattern(pattern: string): Promise<number> {
    try {
      ...
      const result = await this.runQuery(
        "DELETE FROM rag_vectors WHERE id LIKE ? OR file_path LIKE ? OR project_path LIKE ?",
      ...
      VectorStoreLogger.info(
        "deleteByPattern",
        `${deletedCount} documents supprimés avec le pattern: ${pattern}`,
        {
          pattern,
          deletedCount,
        },
      ...
      VectorStoreLogger.error(
        "deleteByPattern",
        `Erreur lors de la suppression avec pattern: ${pattern}`,
        error as Error,
        {
          pattern,
        },
  ...
  async getStats(): Promise<StoreStats> {
    try {
      ...
      const dimensionResult = await this.getQuery(
        `SELECT
      ...
      const averageVectorDimension =
        parseFloat(dimensionResult.avg_dimension) || 0;
  ...
  async initialize(): Promise<void> {
    try {
      ...
      const tableCheck = await this.getQuery(
        "SELECT name FROM sqlite_master WHERE type='table' AND name='rag_vectors'",
      );
      ...
      VectorStoreLogger.info(
        "initialize",
        "Vector store SQLite initialisé avec succès",
      );
    } catch (error) {
      VectorStoreLogger.error(
        "initialize",
    ...
    try {
      ...
      if (connectionOk) {
        ...
        VectorStoreLogger.warn(
          "testConnection",
          "Connexion SQLite testée mais résultat inattendu",
          {
            result,
          },
        );
      ...
      VectorStoreLogger.error(
        "testConnection",
        "Erreur lors du test de connexion SQLite",
        error as Error,
      );
      return false;
    }
  ...
  async updateDocument(
    ...
    try {
      ...
      const existingDoc = await this.getQuery(
        "SELECT id FROM rag_vectors WHERE id = ?",
        [id],
      );
      ...
      if (updates.embedding !== undefined) {
        ...
        const embeddingBuffer = Buffer.from(
          new Float32Array(updates.embedding).buffer,
        );
      ...
      if (updateFields.length === 1) {
        ...
        VectorStoreLogger.warn(
          "updateDocument",
          `Aucune mise à jour à appliquer pour: ${id}`,
          { id },
        );
      ...
      if (updated) {
        VectorStoreLogger.info("updateDocument", `Document mis à jour: ${id}`, {
          id,
          updatedFields: updateFields.filter(
        ...
        VectorStoreLogger.warn(
          "updateDocument",
          `Aucune ligne mise à jour pour: ${id}`,
          { id },
        );
      }
      ...
      VectorStoreLogger.error(
        "updateDocument",
        `Erreur lors de la mise à jour du document ${id}`,
        error as Error,
        { id },
      );
      throw error;
  ...
  async hybridSearch(
    ...
    try {
      ...
      if (semanticWeight + textWeight !== 1.0) {
        VectorStoreLogger.warn(
          "hybridSearch",
          `Somme des pondérations != 1.0 (semantic=${semanticWeight}, text=${textWeight}). Normalisation automatique.`,
      ...
      let textResults: Array<SearchResult & { textScore: number }> = [];
      ...
      if (textQuery.trim()) {
        ...
        if (options?.projectFilter) params.push(options.projectFilter);
        ...
        textResults = rows.map((row) => {
          ...
          const storedEmbedding = Array.from(
            new Float32Array(embeddingBuffer.buffer),
          );
          ...
          return {
            ...
            metadata: {
              ...
              compressionRatio:
                row.is_compressed === 1 && row.original_size_bytes
                  ? (
      ...
      const allResults = new Map<
        string,
        SearchResult & { semanticScore?: number; textScore?: number }
      >();
      ...
      const hybridResults = Array.from(allResults.values()).map((result) => {
        ...
        const hybridScore =
          normalizedSemantic * semanticWeight + normalizedText * textWeight;
  ...
  async searchByMetadata(
    ...
    try {
      ...
      VectorStoreLogger.info(
        "searchByMetadata",
        "Recherche par métadonnées terminée",
        {
  ...
  private cosineSimilarity(vec1: number[], vec2: number[]): number {
    ...
    const [normalizedVec1, normalizedVec2] = this.normalizeVectorLength(
      vec1,
      vec2,
    );
  ...
  async embedAndStore(
    ...
    const finalFileExtension =
      fileExtension || filePath.split(".").pop() || null;
    ...
    try {
      ...
      VectorStoreLogger.error(
        "store",
        `Erreur lors du stockage du document ${id}`,
        error as Error,
        {
          projectPath,
          filePath,
  ...
  async semanticSearch(
    ...
    try {
      ...
      VectorStoreLogger.info(
        "search",
        `Recherche terminée: ${limitedResults.length} résultats`,
        {
  ...
  async getProjectStats(projectPath: string): Promise<ProjectStats> {
    try {
      ...
      VectorStoreLogger.error(
        "stats",
        `Erreur lors de la récupération des stats pour ${projectPath}`,
        error as Error,
      );
      throw error;
    }
  ...
  async deleteDocument(id: string): Promise<boolean> {
    try {
      ...
      VectorStoreLogger.error(
        "delete",
        `Erreur lors de la suppression du document ${id}`,
        error as Error,
      );
      throw error;
    }
  ...
  async clearAll(): Promise<void> {
    try {
      ...
      VectorStoreLogger.error(
        "clear",
        "Erreur lors du vidage de la table",
        error as Error,
      );
      throw error;
    }
