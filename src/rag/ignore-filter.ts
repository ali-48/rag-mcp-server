...
const filterCache = new Map<string, ignore.Ignore>();

export function createIgnoreFilter(projectPath: string): ignore.Ignore {
  // Vérifier le cache
...
export function shouldIgnoreFile(filePath: string, projectPath: string): boolean {
...
export function testIgnoreRules(projectPath: string, testFiles: string[]): Record<string, boolean> {
  const ig = createIgnoreFilter(projectPath);
