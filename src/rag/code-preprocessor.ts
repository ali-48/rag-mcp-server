...
import { ProgrammingLanguage } from './content-detector.js';
...
export interface CodeStructure {
  ...
  comments: Array<{
    type: 'single-line' | 'multi-line' | 'jsdoc' | 'docstring';
    content: string;
    line: number;
  }>;
}
...
function extractComments(
  content: string,
  language: ProgrammingLanguage
): CodeStructure['comments'] {
  const comments: CodeStructure['comments'] = [];
  const lines = content.split('\n');
  
  // Patterns de commentaires par langage
  const commentPatterns: Record<string, { single: RegExp; multiStart: RegExp; multiEnd: RegExp }> = {
 ...
 */
function extractJavaScriptImports(content: string): CodeStructure['imports'] {
 ...
 */
function extractPythonImports(content: string): CodeStructure['imports'] {
  const imports: CodeStructure['imports'] = [];
  const lines = content.split('\n');
 ...
 */
function extractJavaScriptFunctions(content: string): CodeStructure['functions'] {
 ...
 */
function extractPythonFunctions(content: string): CodeStructure['functions'] {
  const functions: CodeStructure['functions'] = [];
  const lines = content.split('\n');
 ...
 */
function extractJavaScriptClasses(content: string): CodeStructure['classes'] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (match) {
      ...
      let braceCount = (line.match(/{/g) || []).length - (line.match(/}/g) || []).length;
      let j = i + 1;
      const methods: CodeStructure['classes'][0]['methods'] = [];
 ...
 */
function extractPythonClasses(content: string): CodeStructure['classes'] {
  const classes: CodeStructure['classes'] = [];
  const lines = content.split('\n');
