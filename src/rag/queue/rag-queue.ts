 ...
 */
export interface RagQueueConfig {
    /** Nombre maximum de jobs dans la file d'attente par projet */
    maxQueueSize: number;

    /** Nombre maximum de jobs mutateurs en cours d'exécution (normalement 1) */
    maxConcurrentMutators: number;

    /** Nombre maximum de jobs en lecture seule en cours d'exécution */
    maxConcurrentReadOnly: number;

    /** Intervalle de vérification des jobs prêts à s'exécuter (ms) */
...
export interface RagQueueStats {
    /** Nombre total de jobs dans la file */
    totalJobs: number;

    /** Nombre de jobs par type */
    jobsByType: Record<RagJobType, number>;

    /** Nombre de jobs par statut */
    jobsByStatus: Record<RagJobStatus, number>;

    /** Nombre de projets dans la file */
    totalProjects: number;

    /** Jobs mutateurs en cours d'exécution */
    runningMutators: number;

    /** Jobs en lecture seule en cours d'exécution */
    runningReadOnly: number;

    /** Taille maximale de la file atteinte */
...
export class RagQueue {
    private jobs = new Map<string, RagJob>();
    private runningJobs = new Set<string>();
    private config: RagQueueConfig;
    private checkInterval: NodeJS.Timeout | null = null;
    private maxQueueSizeReached = 0;

    constructor(config?: Partial<RagQueueConfig>) {
        this.config = {
            maxQueueSize: 10,
     ...
     */
    async enqueue(job: RagJob): Promise<{ queued: boolean; position: number; message?: string }> {
     ...
     */
    private canRunJob(job: RagJob): boolean {
     ...
     * Exécute un job
     */
    private async runJob(job: RagJob): Promise<void> {
     ...
     */
    protected async executeJob(job: RagJob): Promise<void> {
     ...
     */
    getJob(jobId: string): RagJob | null {
     ...
     */
    listJobsByProject(projectPath: string): RagJob[] {
     ...
     */
    listJobsByTypeAndStatus(type?: RagJobType, status?: RagJobStatus): RagJob[] {
     ...
     */
    getStats(): RagQueueStats {
        const jobs = Array.from(this.jobs.values());

        const jobsByType: Record<RagJobType, number> = {
            'scan': 0,
            'prepare': 0,
            'embed': 0,
            'index': 0,
        ...
        const jobsByStatus: Record<RagJobStatus, number> = {
            'pending': 0,
            'running': 0,
            'done': 0,
            'failed': 0,
     ...
     */
    hasRunningJobsForProject(projectPath: string): boolean {
     ...
     */
    hasRunningMutatorsForProject(projectPath: string): boolean {
        return Array.from(this.runningJobs)
            .map(id => this.jobs.get(id))
 ...
 */
function getJobDuration(job: RagJob): number | null {
    if (!job.startedAt) {
        return null;
    }
 ...
 */
let ragQueueInstance: RagQueue | null = null;

/**
 * Obtient l'instance singleton de RagQueue
 */
export function getRagQueue(config?: Partial<RagQueueConfig>): RagQueue {
    if (!ragQueueInstance) {
 ...
 */
export async function testRagQueue(): Promise<boolean> {
    try {
        ...
        const job1 = {
            id: 'test-job-1',
            type: 'scan' as RagJobType,
            status: 'pending' as RagJobStatus,
