...
import { createASTCacheStructure, createCacheDirectories, createCacheReadme, DEFAULT_AST_CACHE_OPTIONS, generateDefaultASTCacheReadme, setupGitignoreForCache } from '../../../scripts/cache-utils/cache-directory-utils';
// Mocks
vi.mock('fs');
vi.mock('path');
describe('cache-directory-utils', () => {
    ...
    describe('setupGitignoreForCache', () => {
        it('should create gitignore file when it does not exist', () => {
            ...
            expect(writeFileSyncMock).toHaveBeenCalledWith(gitignorePath, expect.stringContaining('# Test cache'), 'utf8');
        ...
        it('should append to existing gitignore content', () => {
            ...
            const result = setupGitignoreForCache(mockOptions);
            expect(writeFileSyncMock).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('# Existing content'), 'utf8');
            expect(writeFileSyncMock).toHaveBeenCalledWith(expect.any(String), expect.stringContaining('# Test cache'), 'utf8');
            expect(result).toBe(true);
        });
    });
    describe('createCacheReadme', () => {
        it('should create README file when it does not exist', () => {
            ...
            expect(writeFileSyncMock).toHaveBeenCalledWith(readmePath, expect.stringContaining('# Cache AST - Documentation'), 'utf8');
            expect(result).toBe(true);
        });
    ...
    describe('createASTCacheStructure', () => {
        it('should call all three functions when options are enabled', () => {
            const createCacheDirectoriesSpy = vi.spyOn(require('../../../scripts/cache-utils/cache-directory-utils'), 'createCacheDirectories');
            const setupGitignoreForCacheSpy = vi.spyOn(require('../../../scripts/cache-utils/cache-directory-utils'), 'setupGitignoreForCache');
            const createCacheReadmeSpy = vi.spyOn(require('../../../scripts/cache-utils/cache-directory-utils'), 'createCacheReadme');
...
//# sourceMappingURL=cache-directory-utils.test.js.map