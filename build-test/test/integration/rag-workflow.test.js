...
describe('Workflow d\'intégration RAG', () => {
    ...
    describe('Workflow complet init_rag → activated_rag', () => {
        ...
        it('devrait supporter différents modes d\'initialisation', async () => {
            const modes = ['default', 'memory-only', 'full'];
    ...
    describe('Intégration avec le système de fichiers', () => {
        it('devrait créer la structure de dossiers nécessaire', async () => {
            ...
            expect(mockFs.promises.mkdir).toHaveBeenCalledWith(expect.stringContaining('rag/db/memory'), expect.any(Object));
            expect(mockFs.promises.mkdir).toHaveBeenCalledWith(expect.stringContaining('rag/config'), expect.any(Object));
        });
        it('devrait créer les fichiers de configuration', async () => {
            ...
            expect(mockFs.promises.writeFile).toHaveBeenCalledWith(expect.stringContaining('rag/config/rag.config.json'), expect.any(String), 'utf-8');
            expect(mockFs.promises.writeFile).toHaveBeenCalledWith(expect.stringContaining('rag/config/db.config.json'), expect.any(String), 'utf-8');
            expect(mockFs.promises.writeFile).toHaveBeenCalledWith(expect.stringContaining('.ragignore'), expect.any(String), 'utf-8');
        });
...
//# sourceMappingURL=rag-workflow.test.js.map