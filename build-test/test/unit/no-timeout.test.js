 ...
 */
function checkFileForTimeout(filePath) {
...
describe('Vérification de l\'absence de timeout dans les outils RAG', () => {
    ...
    it('ne devrait pas trouver de timeout dans les fichiers RAG (sauf exceptions)', () => {
        ...
        allFiles.forEach((filePath) => {
            ...
            const isAllowed = ALLOWED_TIMEOUT_FILES.some(allowed => relativePath.includes(allowed) || filePath.includes(allowed));
            if (!isAllowed) {
    ...
    it('devrait permettre les timeout dans query_rag', () => {
        const queryRagFiles = allFiles.filter(file => file.includes('query-rag') && (file.endsWith('.ts') || file.endsWith('.js')));
...
//# sourceMappingURL=no-timeout.test.js.map