...
import { JSONSchemaValidator, validateJsonSchema, } from "../../src/core/json-schema-validator.js";
    ...
    describe("JSONSchemaValidator class", () => {
        ...
        describe("validate - object constraints", () => {
            ...
            it("should validate nested properties", () => {
                ...
                expect(validator.validate({ user: { name: "John", email: "john@example.com" } }, schema).valid).toBe(true);
                expect(validator.validate({ user: { email: "john@example.com" } }, schema)
                    .valid).toBe(false);
            });
            ...
            it("should validate additionalProperties: schema", () => {
                ...
                expect(validator.validate({ name: "John", count: 5 }, schema).valid).toBe(true);
                expect(validator.validate({ name: "John", extra: "value" }, schema).valid).toBe(false);
            });
        });
        describe("validate - array constraints", () => {
            ...
            it("should validate maxLength for arrays", () => {
                ...
                expect(validator.validate(["a", "b"], schema).valid).toBe(true);
                expect(validator.validate(["a", "b", "c"], schema).valid).toBe(true);
                expect(validator.validate(["a", "b", "c", "d"], schema).valid).toBe(false);
            });
        });
        ...
        describe("validate - formats", () => {
            ...
            it("should validate uri format", () => {
                ...
                expect(validator.validate("http://localhost:3000", schema).valid).toBe(true);
                expect(validator.validate("not-a-uri", schema).valid).toBe(false);
            });
            it("should validate date-time format", () => {
                const schema = { type: "string", format: "date-time" };
                expect(validator.validate("2024-01-01T12:00:00Z", schema).valid).toBe(true);
                expect(validator.validate("2024-01-01T12:00:00+01:00", schema).valid).toBe(true);
            ...
            it("should validate uuid format", () => {
                ...
                expect(validator.validate("123e4567-e89b-12d3-a456-426614174000", schema)
                    .valid).toBe(true);
                expect(validator.validate("invalid-uuid", schema).valid).toBe(false);
                expect(validator.validate("123e4567-e89b-12d3-a456-42661417400", schema)
                    .valid).toBe(false); // trop court
            });
        });
