...
import { configureDefaultEmbeddingService, DEFAULT_MODEL_CONFIG, EmbeddingService, generateEmbedding, generateEmbeddingForContent, getDefaultEmbeddingService, getEmbeddingDimensionForModel, getEmbeddingModelForContentType, normalizeL2, } from "../../src/rag/embedding-service.js";
// Mocks
    ...
    describe("EmbeddingService class", () => {
        ...
        describe("getModelForContentType", () => {
            ...
            it("should return code model for code content type", () => {
                ...
                expect(service.getModelForContentType("source")).toBe("nomic-embed-code");
                expect(service.getModelForContentType("program")).toBe("nomic-embed-code");
            });
            it("should return text model for doc content type", () => {
                ...
                expect(service.getModelForContentType("documentation")).toBe("nomic-embed-text");
                expect(service.getModelForContentType("markdown")).toBe("nomic-embed-text");
                expect(service.getModelForContentType("readme")).toBe("nomic-embed-text");
            });
            it("should return config model for config content type", () => {
                expect(service.getModelForContentType("config")).toBe("bge-small");
                expect(service.getModelForContentType("configuration")).toBe("bge-small");
            ...
            it("should return fallback model for unknown content type", () => {
                expect(service.getModelForContentType("unknown")).toBe("qwen3-embedding:8b");
                expect(service.getModelForContentType("other")).toBe("qwen3-embedding:8b");
        ...
        describe("generateForContent", () => {
            it("should use cache when available", async () => {
                ...
                expect(mockEmbeddingCache.get).toHaveBeenCalledWith("test text", "nomic-embed-code");
            ...
            it("should generate and cache when not in cache", async () => {
                ...
                const generateSpy = vi
                    .spyOn(service, "generateWithModel")
                    .mockResolvedValue(generatedEmbedding);
                const result = await service.generateForContent("test text", "code");
                expect(mockEmbeddingCache.get).toHaveBeenCalled();
                expect(generateSpy).toHaveBeenCalledWith("test text", "nomic-embed-code");
        ...
        describe("generateWithModel", () => {
            ...
            it("should fallback to fallback provider when Ollama not configured", async () => {
                const service = new EmbeddingService({
                    provider: "ollama",
                    models: DEFAULT_MODEL_CONFIG,
            ...
            }, 10000); // Timeout 10s pour détection réseau Ollama
        });
        describe("updateConfig", () => {
            ...
            it("should update cache when provided", () => {
                ...
                service.updateConfig({ cache: newCache });
