...
describe('Module A - rag-initialization.ts', () => {
    ...
    describe('A3 - .ragignore', () => {
        it('devrait créer le fichier .ragignore', async () => {
            const result = await initializeRagInfrastructure(mockProjectPath, 'default');
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('.ragignore'), expect.any(String));
        ...
        it('devrait utiliser le template existant', async () => {
            mockFs.existsSync.mockImplementation((p) => p.includes('.ragignore') ? true : false);
            const result = await initializeRagInfrastructure(mockProjectPath, 'default');
            expect(result.details.stepA3.success).toBe(true);
    ...
    describe('A4 - Config RAG', () => {
        it('devrait créer la configuration RAG', async () => {
            const result = await initializeRagInfrastructure(mockProjectPath, 'default');
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('rag-config.json'), expect.any(String));
    ...
    describe('A5 - Config DB', () => {
        it('devrait créer la configuration de base de données', async () => {
            const result = await initializeRagInfrastructure(mockProjectPath, 'default');
            expect(mockFs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('db-config.json'), expect.any(String));
...
//# sourceMappingURL=rag-initialization.test.js.map