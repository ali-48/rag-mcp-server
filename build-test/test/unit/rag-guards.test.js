...
import { checkJobRequirements, checkRagPhase, formatGuardError, getNextPhase, isPhaseReady, requireEmbed, requireIndex, requireInit, requirePrepare, requireQueryReady, requireScan } from '../../src/rag/guards/rag-guards.js';
...
describe('Tests unitaires pour les guards RAG', () => {
    ...
    beforeEach(() => {
        ...
        getRagState.mockResolvedValue(mockRagState);
    });
    afterEach(() => {
        vi.resetAllMocks();
    });
    ...
    describe('formatGuardError', () => {
        it('devrait formater une erreur de guard avec tous les détails', () => {
            const error = new RagUsageError('Prérequis non satisfaits', 'RAG_PHASE_REQUIREMENTS_NOT_MET', {
    ...
    describe('getNextPhase', () => {
        ...
        it('devrait gérer les erreurs et retourner null', async () => {
            ...
            getRagState.mockRejectedValue(new Error('Database error'));
            const nextPhase = await getNextPhase(mockProjectPath);
            expect(nextPhase).toBe(null);
        });
    });
