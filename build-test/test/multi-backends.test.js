...
describe('Multi-Backend Vector Store Tests', () => {
    ...
    describe('Performance Tests', () => {
        ...
        it('should scale with large datasets', async () => {
            ...
            for (let i = 0; i < batchSize; i++) {
                ...
                await store.embedAndStore(projectPath, `/large/dataset/file${i}.txt`, 'x'.repeat(100), // Contenu de 100 caractères
                embedding);
            }
            const endTime = performance.now();
    ...
    describe('Error Handling', () => {
        it('should handle invalid embeddings', async () => {
            ...
            await store.initialize();
            // Embedding vide
            await expect(store.embedAndStore('/test', '/test.txt', 'content', [])).rejects.toThrow();
            await store.clearAll();
        });
