...
export async function getFileHash(filePath) {
    try {
        ...
        if (!fileStats.isFile()) {
            console.warn(`[file-hasher] Le chemin ${filePath} n'est pas un fichier régulier`);
            return null;
        }
        ...
        if (error instanceof Error) {
            if (error.code === "ENOENT") {
                ...
                console.error(`[file-hasher] Erreur lors du calcul du hash pour ${filePath}:`, error.message);
            }
        }
        else {
 ...
 */
export function getStringHash(content) {
    const hash = createHash("md5");
 ...
 */
export async function getFileHashWithMetadata(filePath) {
    try {
        const fileStats = await stat(filePath);
        if (!fileStats.isFile()) {
            return null;
        ...
        console.error(`[file-hasher] Erreur lors du calcul du hash avec métadonnées pour ${filePath}:`, error);
        return null;
    }
}
/**
 ...
 */
export async function compareFilesByHash(filePath1, filePath2) {
    try {
        ...
        console.error(`[file-hasher] Erreur lors de la comparaison des fichiers ${filePath1} et ${filePath2}:`, error);
        return false;
    }
}
/**
 ...
 */
export async function checkFileChange(filePath, previousHash) {
 ...
 */
export async function getFileHashesBatch(filePaths) {
 ...
 */
export async function getFileHashesWithMetadataBatch(filePaths) {
    const promises = filePaths.map((filePath) => getFileHashWithMetadata(filePath));
