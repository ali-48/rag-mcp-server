// src/tools/rag/index-decision-final.ts
...
class RealDecisionIndexer {
     ...
     */
    createMetadataContent(decision) {
...
- Durée: ${decision.decision_data.duration_ms ? `${decision.decision_data.duration_ms}ms` : 'N/A'}`;
    ...
    async indexChunksInVectorStore(chunks) {
        ...
        for (const chunk of chunks) {
            try {
                ...
                await embedAndStore(this.projectPath, filePath, chunk.content, {
     ...
     */
    getDecisionChunks(decisionId) {
        return this.chunks.get(decisionId) || [];
    }
    /**
     * Recherche des décisions similaires
     */
    async searchSimilarDecisions(query, limit = 10) {
        try {
 ...
 */
export const indexDecisionTool = {
    name: "index_decision",
 ...
 */
export const indexDecisionHandler = async (args) => {
    const startTime = Date.now();
    try {
...
//# sourceMappingURL=index-decision-final.js.map