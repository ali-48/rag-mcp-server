...
class TaskContextService {
     ...
     */
    async getTaskContext(taskId, contextType = 'all', limit = 10, similarityThreshold = 0.3) {
     ...
     */
    extractTaskIdFromMetadata(metadata) {
        if (!metadata)
            return undefined;
        ...
        if (metadata.id && metadata.id.startsWith('task-'))
            return metadata.id;
     ...
     */
    extractDecisionIdFromMetadata(metadata) {
        if (!metadata)
            return undefined;
     ...
     */
    extractDecisionTypeFromMetadata(metadata) {
        if (!metadata)
            return undefined;
     ...
     */
    extractDecisionByFromMetadata(metadata) {
        if (!metadata)
            return undefined;
     ...
     */
    extractDecisionTimestampFromMetadata(metadata) {
        if (!metadata)
            return undefined;
     ...
     */
    async findSimilarTasks(taskId, limit, similarityThreshold) {
        try {
            ...
            const similarTasks = Array.from(tasksMap.values())
    ...
    generateRecommendations(contextData) {
        ...
        if (contextData.historical_context && contextData.historical_context.length > 0) {
            ...
            if (completedDecisions.length >= 3) {
                ...
                const longestDecision = completedDecisions.reduce((longest, d) => (d.duration_ms || 0) > (longest.duration_ms || 0) ? d : longest);
            ...
            const successfulDecisions = decisions.filter(d => d.decision_type === 'completed' && !d.error && d.duration_ms && d.duration_ms < 10000);
 ...
 */
export const getTaskContextHandler = async (args) => {
    const startTime = Date.now();
    try {
        ...
        const context = await taskContextService.getTaskContext(args.task_id, args.context_type || 'all', args.limit || 10, args.similarity_threshold || 0.3);
        const endTime = Date.now();
        const duration = endTime - startTime;
        const response = {
            success: true,
 ...
 */
export async function testTaskContextService() {
    try {
