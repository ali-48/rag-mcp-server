...
import { embedAndStore } from "../../rag/vector-store.js";
/**
...
class VSCodeContextManager {
     ...
     */
    createChunksFromContext(context, contextId) {
        const chunks = [];
     ...
     */
    async indexChunksInVectorStore(chunks) {
        let indexedCount = 0;
        for (const chunk of chunks) {
            try {
                // Utiliser les propriétés d'EmbedAndStoreOptions pour stocker les métadonnées
                await embedAndStore(this.projectPath, chunk.id, chunk.content, {
    ...
    createMetadataContent(context) {
        ...
        if (context.metadata) {
            ...
            if (context.metadata.vscode_version)
                lines.push(`- VS Code version: ${context.metadata.vscode_version}`);
            if (context.metadata.extension_version)
                lines.push(`- Extension version: ${context.metadata.extension_version}`);
            if (context.metadata.os)
                lines.push(`- Système d'exploitation: ${context.metadata.os}`);
            if (context.metadata.language)
                lines.push(`- Langue: ${context.metadata.language}`);
            if (context.metadata.timezone)
                lines.push(`- Fuseau horaire: ${context.metadata.timezone}`);
        }
        lines.push('');
     ...
     */
    createWorkspaceContent(workspace) {
        const lines = [];
     ...
     */
    createConfigurationContent(configuration) {
        const lines = [];
        ...
        if (configuration?.workspace_configuration) {
            ...
            if (wc.settings_path)
                lines.push(`- Chemin settings.json: ${wc.settings_path}`);
            if (wc.extensions_json_path)
                lines.push(`- Chemin extensions.json: ${wc.extensions_json_path}`);
        }
        return lines.join('\n');
    }
    /**
    ...
    createGitContent(git) {
        ...
        if (git.repository) {
            ...
            if (repo.root)
                lines.push(`- Racine: ${repo.root}`);
            if (repo.head)
                lines.push(`- Branche courante: ${repo.head}`);
            if (repo.commit)
                lines.push(`- Commit: ${repo.commit?.substring(0, 8) || 'N/A'}`);
            if (repo.upstream)
                lines.push(`- Upstream: ${repo.upstream}`);
            if (repo.ahead || repo.behind)
                lines.push(`- Ahead/Behind: ${repo.ahead || 0} ahead, ${repo.behind || 0} behind`);
            lines.push('');
        }
        if (git.status) {
        ...
        if (git.branches) {
            ...
            if (branches.current)
                lines.push(`- Branche courante: ${branches.current}`);
            if (branches.local?.length) {
                ...
                if (branches.local.length > 5)
                    lines.push(`  * ... et ${branches.local.length - 5} autres`);
            }
            if (branches.remote?.length) {
                ...
                if (branches.remote.length > 3)
                    lines.push(`  * ... et ${branches.remote.length - 3} autres`);
            }
            lines.push('');
        }
        if (git.remotes?.length) {
            ...
            git.remotes.forEach((remote, index) => {
                ...
                if (remote.fetch_url)
                    lines.push(`   Fetch: ${remote.fetch_url}`);
                if (remote.push_url)
                    lines.push(`   Push: ${remote.push_url}`);
            });
        }
        return lines.join('\n');
    }
    ...
    createProjectContent(project) {
        ...
        if (!project?.available) {
            ...
            if (project?.reason)
                lines.push(`Raison: ${project.reason}`);
            return lines.join('\n');
        }
        if (project.root) {
     ...
     */
    createExtensionsContent(extensions) {
        const lines = [];
 ...
 */
export const vscodeContextHandler = async (args) => {
    try {
        const { context } = args;
        if (!context) {
 ...
 */
export async function testVSCodeContextTool() {
    const testContext = {
        metadata: {
