    ...
    async testConnections() {
        ...
        if (this.isAllSqlite()) {
            ...
            Object.keys(results).forEach(key => {
                ...
                results[key] = dbConfig?.type === 'sqlite';
            });
        }
        return results;
    }
 ...
 */
let dbConfigManager = null;
/**
 * Obtient l'instance singleton du gestionnaire de configuration DB
 */
export function getDbConfigManager(configPath) {
    if (!dbConfigManager) {
 ...
 */
export function loadDbConfig(configPath) {
    return getDbConfigManager(configPath).getConfig();
}
/**
 * Test de la configuration DB
 */
export async function testDbConfig() {
    try {
