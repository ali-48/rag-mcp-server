...
export class ProgressTracker {
    tasks = new Map();
    MAX_HISTORY = 1000; // Limite mémoire
    /**
     * Crée une nouvelle tâche de suivi
     */
    create(taskId, projectPath, filesTotal, metadata) {
     ...
     */
    updateEmbeddingCost(taskId, tokens, model, approxSeconds) {
        const task = this.tasks.get(taskId);
        if (!task) {
            return false;
        }
     ...
     */
    updateProgressState(taskId, progressState) {
        const task = this.tasks.get(taskId);
        if (!task) {
 ...
 */
export function generateTaskId(projectPath) {
