...
export class TaskQueue {
     ...
     */
    async waitForCompletion(taskId, timeoutMs = 30000, pollIntervalMs = 500) {
 ...
 */
export function createIndexTask(taskId, projectPath, indexFn, priority = 2) {
    return {
        id: taskId,
        projectPath,
        run: indexFn,
...
export async function testTaskQueue() {
    try {
        ...
        const enqueueResult = await queue.enqueue(testTaskId, testProjectPath, async () => {
            taskExecuted = true;
