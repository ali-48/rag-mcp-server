...
export class OllamaService {
    ...
    async processBatch() {
        ...
        try {
            ...
            VectorStoreLogger.error("ollama.embedding.batch.error", "Failed to process Ollama batch", error);
            // Fallback: traiter chaque texte individuellement
            await this.processIndividualRequests(batch);
        }
    }
     ...
     */
    async processIndividualRequests(batch) {
        for (const item of batch) {
            try {
                ...
                VectorStoreLogger.error("ollama.embedding.individual.error", "Failed to get embedding from Ollama for individual request", error);
     ...
     */
    async generateIndividualEmbedding(text) {
        const response = await fetch(`${this.config.baseUrl}/api/embeddings`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
    ...
    async testConnection() {
        try {
            ...
            VectorStoreLogger.error("ollama.connection.test.error", "Ollama connection test failed", error);
            return false;
        }
    }
    /**
 ...
 */
let defaultOllamaServiceInstance = null;
/**
 * Obtient l'instance singleton du service Ollama
 */
export function getDefaultOllamaService() {
 ...
 */
export function configureDefaultOllamaService(config) {
    defaultOllamaServiceInstance = new OllamaService({
        baseUrl: "http://localhost:11434",
        defaultModel: "qwen3-embedding:8b",
        batchDelayMs: 50,
 ...
 */
export function cleanupDefaultOllamaService() {
    if (defaultOllamaServiceInstance) {
 ...
 */
export async function testDefaultOllamaConnection() {
    return defaultOllamaServiceInstance
