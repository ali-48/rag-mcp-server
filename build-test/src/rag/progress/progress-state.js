...
export function updatePhaseProgress(progressState, phaseId, updates, options = {}) {
    ...
    const totalFilesProcessed = updatedPhases.reduce((sum, phase) => sum + (phase.metrics?.filesProcessed || 0), 0);
    const totalFiles = updatedPhases.reduce((sum, phase) => sum + (phase.metrics?.filesTotal || 0), 0);
    const totalElapsedTime = updatedPhases.reduce((sum, phase) => sum + (phase.metrics?.elapsedTime || 0), 0);
...
export function calculateTimeEstimate(progressState) {
    ...
    const elapsedTime = completedPhases.reduce((sum, phase) => sum + (phase.metrics?.elapsedTime || 0), 0) +
...
export function calculateWorkloadScore(progressState) {
    ...
    const overallScore = Math.round((volumeScore * 0.3) +
        (complexityScore * 0.3) +
