...
export class AutoInitializer {
    statePath;
    backoffConfig;
    constructor(stateDir = '/rag/state') {
    ...
    calculateBackoff(attempt) {
        switch (this.backoffConfig.pattern) {
            ...
            case 'exponential':
                return Math.min(Math.pow(2, attempt - 1) * this.backoffConfig.initialDelayMs, this.backoffConfig.maxDelayMs);
            case 'linear':
                return Math.min(attempt * this.backoffConfig.initialDelayMs, this.backoffConfig.maxDelayMs);
            default:
     ...
     */
    async performInitialization(projectPath) {
        ...
        fs.writeFileSync(path.join(ragDir, 'config.json'), JSON.stringify(projectConfig, null, 2));
        ...
        fs.writeFileSync(path.join(dbLinksDir, 'centralized_databases.json'), JSON.stringify(linkInfo, null, 2));
     ...
     */
    async getInitState(projectPath) {
     ...
     */
    async recordTemporaryFailure(failure) {
        const failuresFile = path.join(this.statePath, 'failures.json');
        let data = { version: '1.0.0', temporary_failures: {} };
     ...
     */
    async cleanupOldFailures(maxAgeDays = 7) {
        const failuresFile = path.join(this.statePath, 'failures.json');
        if (!fs.existsSync(failuresFile))
            return;
        ...
        for (const [projectPath, failure] of Object.entries(data.temporary_failures || {})) {
...
export class PersistentStateManager {
     ...
     */
    async detectProjects(workspaceRoot) {
        const projects = [];
        const signatures = [
     ...
     */
    async scanDirectory(dirPath, maxDepth, callback) {
        if (maxDepth <= 0)
            return;
        try {
     ...
     */
    async saveDetectedProjects(projects) {
        const data = {
            version: '1.0.0',
