...
export class SqliteInitializer {
     ...
     */
    async executeSqlScript(db, scriptPath) {
        try {
     ...
     */
    async initializeMetadataDb(force = false) {
     ...
     * @param force Forcer la réinitialisation même si la base existe déjà
     */
    async initializeMemoryDb(force = false) {
    ...
    async checkDatabaseStatus() {
        ...
        try {
            ...
            if (fs.existsSync(metadataPath)) {
                ...
                const get = promisify(db.get.bind(db));
                const tablesResult = await get("SELECT COUNT(*) as count FROM sqlite_master WHERE type='table'");
                result.metadata.tables = tablesResult?.count || 0;
                await this.closeDatabase(db);
            }
            ...
            if (fs.existsSync(memoryPath)) {
                ...
                const get = promisify(db.get.bind(db));
                const tablesResult = await get("SELECT COUNT(*) as count FROM sqlite_master WHERE type='table'");
                result.memory.tables = tablesResult?.count || 0;
                await this.closeDatabase(db);
            }
        }
        catch (error) {
...
let instance = null;
export function getSqliteInitializer(dbDir) {
    if (!instance) {
        instance = new SqliteInitializer(dbDir);
    }
    return instance;
}
//# sourceMappingURL=sqlite-initializer.js.map