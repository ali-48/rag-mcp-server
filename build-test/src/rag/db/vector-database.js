 ...
 */
async function isPostgreSQLAvailable(config) {
    try {
 ...
 */
async function initializePostgreSQL(config, verbose = false) {
    if (verbose) {
 ...
 */
async function initializeSQLiteVectors(dbDir = '/rag/db', force = false, verbose = false) {
 ...
 */
export async function initializeVectorDatabase(config = {}) {
    const { type = 'postgresql', dbDir = '/rag/db', force = false, verbose = false, postgresql = {
        host: 'localhost',
        port: 5432,
        database: 'rag_vectors',
        user: 'postgres',
        password: 'postgres'
    }, fallbackToSqlite = true } = config;
    if (verbose) {
 ...
 */
export async function isVectorDatabaseInitialized(dbDir = '/rag/db') {
    try {
        const sqliteInitializer = getSqliteInitializer(dbDir);
        const status = await sqliteInitializer.checkDatabaseStatus();
        // Vérifier SQLite
        ...
        if (sqliteAvailable)
            availableTypes.push('sqlite');
        if (postgresqlAvailable)
            availableTypes.push('postgresql');
        return {
 ...
 */
export async function getVectorDatabaseInfo(dbDir = '/rag/db') {
 ...
 */
export async function demonstrateVectorDatabase() {
