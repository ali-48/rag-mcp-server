...
export class CheckpointManager {
    ...
    async init() {
        try {
            ...
            throw new RagUsageError("Impossible d'initialiser le gestionnaire de checkpoints", "CHECKPOINT_MANAGER_INIT_FAILED", {
     ...
     */
    async saveCheckpoint(checkpointId, state, options = {}) {
        const startTime = Date.now();
        try {
            // Vérifier la taille des données
            const stateSize = JSON.stringify(state).length;
            if (stateSize > this.config.maxCheckpointSize) {
                throw new RagUsageError(`Checkpoint trop volumineux: ${stateSize} octets (max: ${this.config.maxCheckpointSize})`, "CHECKPOINT_TOO_LARGE", {
                    details: {
                        stateSize,
            ...
            if (projectCheckpoints >= this.config.maxCheckpointsPerProject) {
                if (!options.force) {
                    throw new RagUsageError(`Limite de checkpoints atteinte pour le projet: ${projectPath}`, "CHECKPOINT_LIMIT_REACHED", {
                        details: {
                            projectPath,
            ...
            throw new RagUsageError("Erreur lors de la sauvegarde du checkpoint", "CHECKPOINT_SAVE_FAILED", {
                cause: error instanceof Error ? error : new Error(String(error)),
     ...
     */
    listCheckpoints(projectPath, type) {
     ...
     */
    async saveToDisk(checkpoint, options) {
     ...
     */
    async loadFromDisk(checkpointId) {
 ...
 */
export async function createJobCheckpoint(job, state, options = {}) {
 ...
 */
export async function restoreJobCheckpoint(jobId) {
    const manager = getCheckpointManager();
 ...
 */
export async function cleanupAllExpiredCheckpoints() {
    const manager = getCheckpointManager();
